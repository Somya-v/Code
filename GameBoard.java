...
public class GameBoard {
	...
	private Dice dice;
	private Queue<Player> nextTurn; // for storing all the players
	private List<Jumper> snakes;
	private List<Jumper> ladders;
	private Map<String, Integer> playersCurrentPosition; // name and position of current player
	int boardSize;
		...
		while (nextTurn.size() > 1) {
			...
			if (nextCell > boardSize) {
				...
				if (nextPosition[0] != nextCell) {
					System.out.println(player.getPlayerName() + " was bitten by the snake at " + nextCell);
				}
				ladders.forEach(v -> {
					if (v.startPoint == nextCell) {
						nextPosition[0] = v.endPoint;
				...
				if(nextPosition[0]!=nextCell && b[0]) {
					System.out.println(player.getPlayerName() + " got ladders at at " + nextCell);
				}
